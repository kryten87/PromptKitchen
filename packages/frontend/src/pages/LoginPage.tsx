  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50">
      <div className="bg-white shadow-md rounded-lg p-8 w-full max-w-md flex flex-col items-center">
        ...
        <button
          className="w-full flex items-center justify-center gap-2 py-3 px-4 border border-gray-300 rounded-md bg-white hover:bg-gray-100 transition text-gray-800 font-medium shadow-sm"
          onClick={() => {
            // Use backend URL for OAuth: if running on localhost:3000, use localhost:3001; otherwise, use same origin
            const isLocal = window.location.hostname === 'localhost' && window.location.port === '3000';
            const backend = isLocal ? 'http://localhost:3001' : window.location.origin;
